 ...
 *
 * Features:
 * - View all Packages & Add-Ons test files organized by category
 * - Run individual test files or full test suites
 * - Display test results in real-time
 * - Test status tracking (passed/failed/pending)
 *
 * @tier ADVANCED
 * @phase Phase 4 - Packages & Add-Ons Integration & Testing
 * @authority docs/packages/phases/PHASE_4_BRAIN_PLAN.md
...
const getInitialTestCategories = (): TestCategory[] => [
  {
    ...
    description: 'Admin packages page E2E tests - CRUD, password verification, status toggle (12 tests)',
    icon: Shield,
    expanded: true,
    files: [
      {
        id: 'admin-packages',
        name: 'packages.spec.ts',
        path: 'e2e/admin/packages.spec.ts',
        testCount: 12,
        status: 'idle'
      }
    ]
  ...
  {
    id: 'e2e-public',
    name: 'Public E2E Tests',
    description: 'Public memberships page E2E tests - display, pricing, CTAs, SEO (21 tests)',
    icon: Globe,
    expanded: true,
    files: [
      {
        id: 'public-memberships',
        name: 'memberships.spec.ts',
        path: 'e2e/public/memberships.spec.ts',
        testCount: 21,
        status: 'idle'
      }
    ]
  },
  {
    ...
    description: 'Packages & Add-ons API tests - CRUD, status toggle, pagination (24 tests)',
    icon: Server,
    expanded: true,
    files: [
      {
        id: 'packages-api',
        name: 'packages-api.test.ts',
        path: 'tests/integration/packages-api.test.ts',
        testCount: 24,
        status: 'idle'
      }
    ]
...
function PackagesTestDashboardContent() {
  const { user } = useAuth();
  ...
  const runAllTests = useCallback(async () => {
    ...
    setTestOutput('[Starting] Running all Packages & Add-Ons tests...\n');
  ...
  const apiTests = categories
    .filter(c => c.id === 'api-integration')
    .reduce((sum, c) => sum + c.files.reduce((s, f) => s + f.testCount, 0), 0);
  const totalExecutions = (e2eTests * 5) + apiTests;
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
              <Package className="w-6 h-6 text-[#ed6437]" />
              Packages & Add-Ons Test Dashboard
            </h1>
            <p className="text-sm text-gray-600">
              Phase 4 Integration & Testing - E2E and API Tests
            </p>
          </div>
          <div className="flex items-center gap-3">
            <button
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          ...
          <p className="text-sm text-blue-700">
            Tests follow the Phase 4 Brain Plan specification and use patterns from MEMORY.md.
          </p>
          <div className="flex flex-col sm:flex-row gap-2 mt-2">
            <p className="text-xs text-blue-600 font-mono">
              docs/packages/phases/PHASE_4_BRAIN_PLAN.md
            </p>
            <span className="hidden sm:inline text-blue-400">|</span>
            <p className="text-xs text-blue-600 font-mono">
              docs/packages/phases/README.md
            </p>
          </div>
        </div>
        ...
        <div className="bg-gray-100 border border-gray-200 rounded-lg p-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 font-mono text-xs text-gray-700">
            <div className="bg-white rounded px-3 py-2">
              <span className="text-gray-500"># All E2E tests</span>
              <p>npx playwright test</p>
            </div>
            <div className="bg-white rounded px-3 py-2">
              <span className="text-gray-500"># Admin tests only</span>
              <p>npx playwright test e2e/admin</p>
            </div>
            <div className="bg-white rounded px-3 py-2">
              <span className="text-gray-500"># Public tests only</span>
              <p>npx playwright test e2e/public</p>
            </div>
            <div className="bg-white rounded px-3 py-2">
              <span className="text-gray-500"># API integration tests</span>
              <p>npm run test:integration:vitest</p>
            </div>
          </div>
        </div>
      </div>
...
export default function PackagesTestDashboardPage() {
  return (
    <ErrorBoundary componentName="PackagesTestDashboard">
      <PackagesTestDashboardContent />
    </ErrorBoundary>
  );
}
