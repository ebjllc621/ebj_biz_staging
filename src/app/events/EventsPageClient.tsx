/**
 * EventsPageClient - Client Component for Events Page
 *
 * Handles client-side interactivity including data fetching, state management,
 * and URL query parameter synchronization. Renders event card grid with
 ...
 *
 * FEATURES (Phase 2):
 * - State management for events, loading, errors, pagination
 * - Data fetching from /api/events/search
 * - URL query parameter synchronization
 * - Loading skeletons and error states
 * - Placeholder for event cards (Phase 4)
 *
...
import { AlertCircle, Clock, WifiOff, Calendar } from 'lucide-react';
import type { EventWithCoordinates, EventDisplayMode } from '@/features/events/types';

import { EventCard } from '@/features/events/components/EventCard';
import { EventCardHorizontal } from '@/features/events/components/EventCardHorizontal';
import dynamic from 'next/dynamic';

const EventsMap = dynamic(
  () => import('@/features/events/components/EventsMap').then(mod => mod.EventsMap),
  {
    ssr: false,
    loading: () => (
...
const EventsFilterBar = dynamic(
  () => import('@/features/events/components/EventsFilterBar').then(mod => ({ default: mod.EventsFilterBar })),
  { ssr: true, loading: () => <div className="h-12 bg-gray-100 animate-pulse rounded" /> }
);

const EventsCampaignSection = dynamic(
  () => import('@/features/events/components/EventsCampaignSection').then(mod => ({ default: mod.EventsCampaignSection })),
  ...
  return (
    <div className="bg-gray-50 border border-gray-200 rounded-lg p-8 text-center">
      ...
      <p className="text-gray-600">
        Try adjusting your search or filters to find upcoming events.
      </p>
    </div>
  );
}
...
/**
 * EventsPageClient - Main client component for events page
 */
export function EventsPageClient() {
  ...
  const [displayMode, setDisplayMode] = useState<EventDisplayMode>('grid');
  ...
  const currentDisplayMode = (searchParams.get('view') as EventDisplayMode) || 'grid';
   ...
   * [PHASE_3_API]: Will fetch from /api/events/search
   */
  const fetchEvents = useCallback(async () => {
    try {
      ...
      const result: EventsSearchResponse = await response.json();
  ...
  // Fetch events when URL params change
  useEffect(() => {
    fetchEvents();
  }, [fetchEvents]);
   ...
   */
  const handleDisplayModeChange = useCallback((mode: EventDisplayMode) => {
    setDisplayMode(mode);
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600">
          Discover upcoming events, workshops, and community gatherings.
        </p>
      </div>

      {/* Filter Bar with integrated view controls */}
      <EventsFilterBar
        className="mb-6"
      ...
      <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="hidden lg:block lg:w-1/2 xl:w-2/5">
            <div className="sticky top-4">
              <EventsMap
                ...
                onMarkerHover={setHighlightedEventId}
        ...
        <div className="flex-1">
          ...
          {/* Loading State */}
          {isLoading && (
            displayMode === 'grid' ? <EventsSkeleton /> : <EventsListSkeleton />
          )}
          ...
          {/* Empty State */}
          {!isLoading && !error && events.length === 0 && <EventsEmpty />}
      ...
      {/* Campaign Promotions */}
      <EventsCampaignSection className="mt-12" />
    </div>
  );
}
