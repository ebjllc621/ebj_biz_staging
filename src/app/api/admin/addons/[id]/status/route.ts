/**
 * Admin Addon Status Toggle API Route
 * PATCH /api/admin/addons/[id]/status - Toggle addon status (active/inactive)
 *
 ...
 * Toggle addon status between active and inactive immediately
 */
export const PATCH = withCsrf(apiHandler(async (context) => {
  ...
  // Path is /api/admin/addons/[id]/status, so [id] is 3rd from end
  ...
  const updated = await service.toggleAddonStatus(addonId);
  ...
  await adminActivityService.logActivity({
    ...
    actionType: 'addon_status_updated',
    actionCategory: 'update',
    actionDescription: `Toggled status for addon ID: ${addonId}`,
