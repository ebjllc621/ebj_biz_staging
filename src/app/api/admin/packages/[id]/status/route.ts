/**
 * Admin Package Status Toggle API Route
 * PATCH /api/admin/packages/[id]/status - Toggle package status (active/inactive)
 *
 * GOVERNANCE COMPLIANCE:
 * - apiHandler wrapper: MANDATORY
 * - Authentication: Admin-only
 * - CSRF protection: PATCH operation
 * - Immediate effect: No password verification required for status toggle
 *
 ...
 * Toggle package status between active and inactive immediately
 */
export const PATCH = withCsrf(apiHandler(async (context) => {
  ...
  // Path is /api/admin/packages/[id]/status, so [id] is 3rd from end
  ...
  const updated = await service.togglePlanStatus(packageId);
  ...
  await adminActivityService.logActivity({
    ...
    actionType: 'package_status_updated',
    actionCategory: 'update',
    actionDescription: `Toggled status for package ID: ${packageId}`,
