 ...
 * Phase 4 - Send thank you message to recommendation sender
 *
 * @tier SIMPLE
...
export const POST = apiHandler(async (context: ApiContext) => {
  ...
  try {
    ...
    const bodyValidation = validateInput(SendThankYouSchema, body);
    ...
    const result = await sharingService.sendThankYou(userId, recommendationId, message);
    ...
    return createErrorResponse(
      new BizError({ code: 'INTERNAL_ERROR', message: 'Failed to send thank you' }),
      context.requestId
    );
  }
}, { requireAuth: true });
