/**
 * Internet Personality Contact Proposal API Route
 * POST /api/content/internet-personalities/[slug]/contact
 *
...
export const POST = apiHandler(async (context: ApiContext, routeParams?: RouteParams) => {
  ...
  try {
    const { proposalId, profileOwnerId } = await personalityService.createContactProposal(
      slug,
      senderUserId,
      senderName,
      senderEmail,
    ...
    notificationService.dispatch({
      ...
      title: 'New collaboration proposal received',
      message: `${senderName} wants to collaborate with you: "${subject.trim()}"`,
      entityType: 'content',
      entityId: proposalId,
