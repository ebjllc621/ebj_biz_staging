/**
 * Podcast Detail API Route
 * GET /api/content/podcasts/[slug] - Single podcast by slug
 *
...
export const GET = apiHandler(async (context: ApiContext, routeParams?: RouteParams) => {
  ...
  try {
    ...
    if (!podcast) {
      throw new ContentNotFoundError('podcast', slug);
    }

    // Increment view count (fire and forget)
    contentService.incrementViewCount('podcast', podcast.id).catch(() => {
      // Silent failure - view count is non-critical
    });

    return createSuccessResponse({ podcast }, context.requestId);
  } catch (error) {
