/**
 * ListingShareModal - Social sharing modal for listings
 *
...
interface ListingShareModalProps {
  ...
  /** Share context - post-publish or general share */
  context?: 'post-publish' | 'share';
}
...
/** Strip HTML tags to get plain text for preview display, preserving spacing between blocks.
 *  Also removes leading heading text (e.g. "About Business Name") since the listing name is already shown. */
function stripHtml(html: string): string {
  return html
    .replace(/^(\s*<h[1-6][^>]*>.*?<\/h[1-6]>\s*)+/i, '')
    .replace(/<\/(h[1-6]|p|div|li|br|blockquote)[^>]*>/gi, ' ')
    .replace(/<[^>]*>/g, '')
    .replace(/\s+/g, ' ')
    .trim();
}

function ListingShareModalInner({
  isOpen,
  onClose,
  listing,
  context = 'share'
  ...
  // Build listing URL with UTM parameters
  const buildShareUrl = useCallback((platform: string) => {
    ...
    const url = new URL(`${baseUrl}/listings/${listing.slug}`);

    url.searchParams.set('utm_source', platform);
    url.searchParams.set('utm_medium', 'social');
    url.searchParams.set('utm_campaign', 'listing_share');
    url.searchParams.set('utm_content', listing.slug);
  ...
  const getShareText = useCallback(() => {
    return `Check out ${listing.name} on Bizconekt!`;
  }, [listing.name]);
  ...
  const handleShare = useCallback((platform: string) => {
    ...
    fetch(`/api/listings/${listing.id}/shares`, {
      method: 'POST',
    ...
    switch (platform) {
      ...
      case 'sms':
        ...
        break;
      case 'email':
        targetUrl = `mailto:?subject=${encodeURIComponent(`Check out ${listing.name}`)}&body=${encodeURIComponent(`${shareText}\n\n${shareUrl}`)}`;
        break;
    }
  ...
  }, [buildShareUrl, getShareText, listing.id, listing.name]);
  ...
  }, [buildShareUrl, listing.id]);

  const title = context === 'post-publish' ? 'Your Listing is Live!' : 'Share This Listing';
  const subtitle = context === 'post-publish'
    ? 'Help others discover your business'
    : 'Spread the word about this business';
      ...
      <div className="space-y-6">
        ...
        <div className="flex gap-4 p-4 bg-gray-50 rounded-lg">
          ...
          <div className="flex-1 min-w-0">
            ...
            {listing.description && (
              <p className="text-sm text-gray-600 line-clamp-2">{stripHtml(listing.description)}</p>
            )}
          </div>
        </div>
...
/**
 * ListingShareModal with ErrorBoundary (STANDARD tier requirement)
 */
export function ListingShareModal(props: ListingShareModalProps) {
  return (
    <ErrorBoundary componentName="ListingShareModal">
      <ListingShareModalInner {...props} />
    </ErrorBoundary>
  );
}
