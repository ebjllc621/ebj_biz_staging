/**
 * SidebarSocialCard - Compact Social Media Icons
 *
 ...
 *
 * Features:
 * - Social media icon-only display (no labels)
 * - Compact horizontal layout
 * - Platform-specific colors on hover
 * - Returns null if no social links
 *
...
interface SidebarSocialCardProps {
  /** Listing data */
  listing: Listing;
}

// Social media platform definitions with SVG icons (from ListingSocialLinks)
...
export function SidebarSocialCard({ listing }: SidebarSocialCardProps) {
  ...
  // Return null if no social links
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm overflow-hidden p-3">
      ...
      <h4 className="text-sm font-semibold text-gray-900 flex items-center gap-2 mb-3">
        <Share2 className="w-4 h-4 text-biz-orange" />
        Social Links
      </h4>
      ...
      <div className="flex flex-wrap gap-2">
        {activePlatforms.map(platform => {
          ...
          return (
            ...
            >
              <span className="text-gray-500 group-hover:text-[var(--platform-color)] transition-colors">
                {platform.icon}
              </span>
            </a>
          );
...
export default SidebarSocialCard;
