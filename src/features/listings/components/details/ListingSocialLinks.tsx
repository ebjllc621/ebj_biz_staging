/**
 * ListingSocialLinks - Social Media Links Display
 *
...
import type { Listing, SocialMedia } from '@core/services/ListingService';

interface ListingSocialLinksProps {
  /** Listing data */
  listing: Listing;
  isEditMode?: boolean;
}

// Social media platform definitions with SVG icons
...
export function ListingSocialLinks({ listing, isEditMode }: ListingSocialLinksProps) {
  // Parse social media from listing
  ...
  const hasSocialLinks = activePlatforms.length > 0;

  // Show empty state in edit mode when no social links
  if (isEditMode && !hasSocialLinks) {
    return (
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-gray-500 mb-3">
              No social media links yet. Add your social media profiles.
            </p>
            <Link
              href={`/dashboard/listings/${String(listing.id)}/social-media` as any}
  ...
  // Return null in published mode when no social links
  if (!hasSocialLinks) {
    return null;
  }
        ...
        {activePlatforms.map(platform => {
          ...
          return (
            <a
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-lg border border-gray-200 hover:border-gray-300 hover:shadow-sm transition-all group"
              aria-label={`Visit ${listing.name} on ${platform.label}`}
              style={{ '--platform-color': platform.color } as React.CSSProperties}
            >
              <span
                className="text-gray-500 group-hover:text-[var(--platform-color)] transition-colors"
              >
                {platform.icon}
              </span>
