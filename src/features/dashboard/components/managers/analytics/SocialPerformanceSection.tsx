/**
 * SocialPerformanceSection - Share performance analytics for a listing
 *
 ...
 *
 * Replicates OfferShareAnalyticsPanel pattern adapted for listings.
 * Self-contained: fetches own data from /api/listings/[id]/shares.
 * Key difference: "Contacts" instead of "Claims" in conversions section.
 */
'use client';
...
export interface SocialPerformanceSectionProps {
  listingId: number;
  listingName?: string;
}
...
interface ShareAnalytics {
  ...
  shares_by_platform: Partial<Record<SharePlatform, number>>;
  clicks_by_platform: Partial<Record<SharePlatform, number>>;
  conversions: {
    signups: number;
    contacts: number;
  };
...
export function SocialPerformanceSection({ listingId, listingName }: SocialPerformanceSectionProps) {
  ...
  const fetchShareAnalytics = useCallback(async () => {
    ...
    try {
      ...
      // API returns { data: { analytics: {...} } } based on offers pattern
      const analyticsData = result.data?.analytics ?? result.data ?? null;
    ...
    return (
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        ...
        <div className="bg-red-50 text-red-600 p-4 rounded-lg flex items-center justify-between">
          <span>{error}</span>
          <button
            onClick={() => void fetchShareAnalytics()}
    ...
    return (
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        ...
        <div className="text-center py-8 text-gray-500">
          ...
          <p>No shares yet.</p>
          <p className="text-sm mt-1">Share this listing on social media to track performance.</p>
        </div>
      </div>
    );
  }
  ...
  const chartData = Object.entries(analytics.shares_by_platform)
    ...
    .map(([platform, shares]) => ({
      ...
      clickRate: (shares ?? 0) > 0
        ? ((analytics.clicks_by_platform[platform as SharePlatform] ?? 0) / (shares ?? 1)) * 100
        : 0
    }))
    .sort((a, b) => b.shares - a.shares);

  const hasConversions =
    (analytics.conversions.signups > 0) || (analytics.conversions.contacts > 0);
    ...
    <div className="bg-white rounded-lg border border-gray-200 p-6">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          {listingName && (
            <p className="text-sm text-gray-500 mt-0.5">{listingName}</p>
          )}
        </div>
        <button
          onClick={() => void fetchShareAnalytics()}
        ...
        {chartData.map(item => (
          <div key={item.platform} className="flex items-center justify-between p-2 bg-gray-50 rounded-lg">
            ...
            <div className="text-sm text-gray-600">
              {item.shares} shares &middot; {item.clicks} clicks &middot;
        ...
        <div className="mt-6 pt-4 border-t border-gray-200">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="bg-green-50 rounded-lg p-3">
              <p className="text-sm text-green-600">Contacts</p>
              <p className="text-xl font-bold text-gray-900">{analytics.conversions.contacts}</p>
            </div>
          </div>
        </div>
      )}
...
export default SocialPerformanceSection;
