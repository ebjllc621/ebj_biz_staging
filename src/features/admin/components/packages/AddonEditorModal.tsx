/**
 * AddonEditorModal - Create/Edit Add-On Suites
 *
 * @authority docs/packages/phases/PHASE_2_BRAIN_PLAN.md
 * @tier STANDARD
 *
 * Features:
 * - Create new add-on suites
 * - Edit existing add-ons (password collected via modal popup on save)
 * - Rich text description using TipTapEditor
 * - Form validation
 * - Suite name selection dropdown
...
import { AddonSuiteName } from '@core/types/subscription';
import type { AdminAddonSuite, CreateAddonInput, UpdateAddonInput } from '@/types/admin-packages';
...
interface AddonEditorModalProps {
  ...
  onClose: () => void;
  onSave: () => void;
  addon?: AdminAddonSuite | null; // null = create mode
}
...
const SUITE_OPTIONS: { value: AddonSuiteName; label: string }[] = [
  { value: AddonSuiteName.CREATOR, label: 'Creator Suite' },
  { value: AddonSuiteName.REALTOR, label: 'Realtor Suite' },
  { value: AddonSuiteName.RESTAURANT, label: 'Restaurant Suite' },
  { value: AddonSuiteName.SEO_SCRIBE, label: 'SEO Scribe Suite' },
];

const INITIAL_FORM_STATE: FormState = {
  display_name: '',
  description: '',
  suite_name: AddonSuiteName.CREATOR,
  version: '1.0.0',
  pricing_monthly: '',
  pricing_annual: '',
  features: '',
...
export function AddonEditorModal({
  ...
  const isEditMode = !!existingAddon;
  ...
  useEffect(() => {
    if (isOpen) {
      if (existingAddon) {
        setFormState({
          display_name: existingAddon.display_name,
          description: existingAddon.description || '',
          suite_name: existingAddon.suite_name,
          version: existingAddon.version,
          pricing_monthly: existingAddon.pricing_monthly?.toString() || '',
          pricing_annual: existingAddon.pricing_annual?.toString() || '',
          features: existingAddon.features.join('\n'),
          effective_date: existingAddon.effective_date.split('T')[0] ?? '',
        });
      } else {
  ...
  const parseFeatures = useCallback((featuresText: string): string[] => {
    return featuresText
      .split('\n')
      .map(f => f.trim())
      .filter(f => f.length > 0);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      const createData: CreateAddonInput = {
        display_name: formState.display_name.trim(),
        suite_name: formState.suite_name,
      ...
      const response = await fetchWithCsrf('/api/admin/addons', {
        method: 'POST',
        credentials: 'include',
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error?.message || 'Failed to create add-on');
      }
  ...
  }, [validateForm, isEditMode, formState, parseFeatures, onSave, onClose]);
  ...
  const handlePasswordSubmit = useCallback(async () => {
    ...
    try {
      // Update existing addon (password included for verification)
      const updateData = {
        display_name: formState.display_name.trim(),
        features: parseFeatures(formState.features),
      ...
      const response = await fetchWithCsrf(`/api/admin/addons/${existingAddon.id}`, {
        method: 'PUT',
        credentials: 'include',
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error?.message || 'Failed to update add-on');
      }
  ...
  }, [passwordInput, existingAddon, formState, parseFeatures, onSave, onClose]);
  ...
  <>
    <BizModal
      isOpen={isOpen}
      onClose={onClose}
      title={isEditMode ? 'Edit Add-On Suite' : 'Create Add-On Suite'}
      subtitle={isEditMode ? `Editing ${existingAddon?.display_name}` : 'Add a new add-on suite'}
      maxWidth="lg"
      footer={
        <div className="flex justify-end gap-3">
          ...
          >
            {isSubmitting ? 'Saving...' : isEditMode ? 'Save Changes' : 'Create Add-On'}
          </BizModalButton>
        </div>
      }
    >
      <div className="space-y-6">
        ...
        <div>
          ...
          <input
            ...
            placeholder="Enter add-on display name"
            className={`w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${errors.display_name ? 'border-red-500' : 'border-gray-300'}`}
          />
          {errors.display_name && <p className="mt-1 text-sm text-red-500">{errors.display_name}</p>}
        </div>

        {/* Suite Name and Version Row */}
        <div className="grid grid-cols-2 gap-4">
          {/* Suite Name Field - Only for create mode */}
          <div>
            <label htmlFor="addon-suite" className="block text-sm font-medium text-gray-700 mb-1">
              Suite Type {!isEditMode && <span className="text-red-500">*</span>}
            </label>
            <select
              id="addon-suite"
              value={formState.suite_name}
              onChange={(e) => handleFieldChange('suite_name', e.target.value)}
              disabled={isEditMode}
            ...
            {isEditMode && (
              <p className="mt-1 text-xs text-gray-500">Suite type cannot be changed after creation</p>
            )}
          </div>

          {/* Version Field */}
          <div>
            <label htmlFor="addon-version" className="block text-sm font-medium text-gray-700 mb-1">
        ...
        {/* Description Field */}
        <div>
          <label htmlFor="addon-description" className="block text-sm font-medium text-gray-700 mb-1">
            Description
          </label>
          <TipTapEditor
        ...
        {/* Features Field */}
        <div>
          <label htmlFor="addon-features" className="block text-sm font-medium text-gray-700 mb-1">
          ...
          <p className="mt-1 text-xs text-gray-500">
            {parseFeatures(formState.features).length} features
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label htmlFor="addon-pricing-monthly" className="block text-sm font-medium text-gray-700 mb-1">
              Monthly Price ($)
            </label>
            <input
              id="addon-pricing-monthly"
              type="number"
              min="0"
              step="0.01"
          ...
          <div>
            <label htmlFor="addon-pricing-annual" className="block text-sm font-medium text-gray-700 mb-1">
              Annual Price ($)
            </label>
            <input
              id="addon-pricing-annual"
              type="number"
              min="0"
              step="0.01"
        ...
        {!isEditMode && (
          <div>
            <label htmlFor="addon-effective-date" className="block text-sm font-medium text-gray-700 mb-1">
              Effective Date <span className="text-red-500">*</span>
            </label>
            <input
              id="addon-effective-date"
              type="date"
    ...
    {isEditMode && (
      ...
      >
        <form onSubmit={(e) => { e.preventDefault(); handlePasswordSubmit(); }} className="p-6">
          ...
          <div className="mb-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg flex gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-yellow-700 mt-1">
                You are about to <span className="font-semibold">save changes to {existingAddon?.display_name}</span>.
          ...
          {/* Password Input */}
          <div className="mb-4">
            <label htmlFor="addon-admin-password" className="block text-sm font-medium text-gray-700 mb-2">
              Admin Password
            </label>
            <div className="relative">
              <input
                id="addon-admin-password"
          ...
          <div className="mb-6 p-3 bg-gray-50 rounded-lg">
            <p className="text-xs text-gray-600">
              ...
              This verification is required for all add-on modifications.
            </p>
          </div>
