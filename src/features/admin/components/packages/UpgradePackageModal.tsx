...
import type { AdminSubscriptionPlan } from '@/types/admin-packages';
...
export function UpgradePackageModal({
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetchWithCsrf(`/api/admin/packages/${existingPackage.id}/upgrade`, {
        method: 'POST',
        credentials: 'include',
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error?.message || 'Failed to upgrade package');
      }
  ...
  }, [validateForm, formState, existingPackage.id, onSuccess, onClose]);
  ...
  return (
    <BizModal
      ...
      subtitle={`Creating new version for ${existingPackage.name} (${existingPackage.tier})`}
      maxWidth="lg"
      footer={
    ...
    >
      <div className="space-y-6">
        ...
        <div className="bg-yellow-50 border border-yellow-200 text-yellow-800 px-4 py-3 rounded-md">
          <p className="text-sm">
            <strong>Note:</strong> Creating a new version will automatically hide the current displayed version for this tier.
        ...
        <div className="bg-gray-50 p-4 rounded-md space-y-2">
          <p className="text-sm text-gray-600">Current Version: <span className="font-medium text-gray-900">{existingPackage.version}</span></p>
          <p className="text-sm text-gray-600">Current Monthly: <span className="font-medium text-gray-900">${existingPackage.pricing_monthly || 'N/A'}</span></p>
          <p className="text-sm text-gray-600">Current Annual: <span className="font-medium text-gray-900">${existingPackage.pricing_annual || 'N/A'}</span></p>
        </div>
